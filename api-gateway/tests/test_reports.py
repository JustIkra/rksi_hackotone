...
@pytest.mark.unit
@patch("app.routers.reports.extract_images_from_report.delay")
...
# Test: Get Report Images


@pytest_asyncio.fixture
async def report_with_images(
    db_session: AsyncSession, participant: Participant
) -> tuple[Report, list[tuple[FileRef, Path]]]:
    """
    Create a test report with sample images.

    Returns:
        Tuple of (Report, list of (FileRef, Path) tuples)
    """
    from app.db.models import ReportImage

    # Create file reference for report
    ...
    report = Report(
        ...
        participant_id=participant.id,
        status="EXTRACTED",
        file_ref_id=report_file_ref.id,
        uploaded_at=datetime.now(UTC),
        extracted_at=datetime.now(UTC),
    )

    db_session.add(report_file_ref)
    db_session.add(report)
    await db_session.flush()

    # Create sample image files and references
    image_files = []
    for i in range(2):
        ...
        # Create file reference for image
        image_file_ref = FileRef(
            id=uuid.uuid4(),
            storage="LOCAL",
            bucket="local",
            key=f"reports/{participant.id}/{report.id}/image_{i}.png",
        ...
        base_path = Path(settings.file_storage_base)
        file_path = base_path / image_file_ref.key
        file_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        report_image = ReportImage(
            id=uuid.uuid4(),
            report_id=report.id,
            file_ref_id=image_file_ref.id,
            kind="TABLE",
            page=i,
            order_index=i,
        )

        db_session.add(image_file_ref)
        db_session.add(report_image)
        image_files.append((image_file_ref, file_path))

    await db_session.commit()
    await db_session.refresh(report)

    return report, image_files


@pytest.mark.unit
async def test_get_report_images_success(
    user_client: AsyncClient,
    report_with_images: tuple[Report, list[tuple[FileRef, Path]]],
):
    """
    GIVEN a report with extracted images
    WHEN requesting images via API
    THEN returns list of images as base64 data URLs
    """
    report, image_files = report_with_images

    response = await user_client.get(f"/api/reports/{report.id}/images")
    ...
    assert first_image["filename"] == "image_0.png"
    assert first_image["data_url"].startswith("data:image/png;base64,")
    assert first_image["page_number"] == 0

    # Verify second image
    second_image = data[1]
    assert second_image["filename"] == "image_1.png"
    assert second_image["data_url"].startswith("data:image/png;base64,")
    assert second_image["page_number"] == 1
...
@pytest.mark.unit
async def test_get_report_images_empty(
    user_client: AsyncClient,
    report_with_file: tuple[Report, FileRef, Path],
):
    """
    GIVEN a report with no images
    WHEN requesting images via API
    THEN returns empty list
    """
    report, _, _ = report_with_file
...
@pytest.mark.unit
async def test_get_report_images_not_found(
    user_client: AsyncClient,
):
    """
    GIVEN a non-existent report ID
    ...
    """
    fake_id = uuid.uuid4()
    response = await user_client.get(f"/api/reports/{fake_id}/images")

    assert response.status_code == 404


@pytest.mark.unit
async def test_get_report_images_unauthenticated(
    client: AsyncClient,
    report_with_images: tuple[Report, list[tuple[FileRef, Path]]],
):
    """
    GIVEN an unauthenticated user
    WHEN requesting report images
    THEN returns 401 Unauthorized
    """
    report, _ = report_with_images

    response = await client.get(f"/api/reports/{report.id}/images")
